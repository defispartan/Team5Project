...
"""
Loads and pre-preocess the 20 bAbI task. It also augments task 16 as described in paper.
"""


DEFAULT_DATA_FOLDER = "bAbI"
            ...
            if (self.data_dir / subset).exists():
                for file_name in (self.data_dir / subset).iterdir():
                    ...
                    try:
                        ...
                        task, set = file_name.split("_")
                        task_name = 'N/A0'
                    if task == "qa" + str(task_no):
                        if set == 'test.txt' or set == 'valid.txt':
