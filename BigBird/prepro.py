...
def build_winograd(data, dump_path, max_seq_len=MAX_SEQ_LEN):
    """Build Winograd as a pair-wise ranking task
    """
    with open(dump_path, 'w', encoding='utf-8') as writer:
        for idx, sample in enumerate(data):
            ids = sample['uid']
...
def build_copa(data, dump_path, max_seq_len=MAX_SEQ_LEN):
    """Build COPA as a pair-wise ranking task
    """
    with open(dump_path, 'w', encoding='utf-8') as writer:
        for idx, sample in enumerate(data):
            ids = sample['uid']
            if len(sample['hypothesis']) == 2: # For effect problems
                premise = bert_tokenizer.tokenize(sample['premise'])
                hypothesis_1 = bert_tokenizer.tokenize(sample['hypothesis'][0])
                hypothesis_2 = bert_tokenizer.tokenize(sample['hypothesis'][1])
                label = sample['label']
            ...
            elif len(sample['premise']) == 2: # For cause problems
                premise_1 = bert_tokenizer.tokenize(sample['premise'][0])
                premise_2 = bert_tokenizer.tokenize(sample['premise'][1])
                hypothesis = bert_tokenizer.tokenize(sample['hypothesis'])
                label = sample['label']

                _truncate_seq_pair(premise_1, hypothesis, max_seq_len - 3)
                input_ids_1 = bert_tokenizer.convert_tokens_to_ids(['[CLS]'] + hypothesis + ['[SEP]'] + premise_1 + ['[SEP]'])
                type_ids_1 = [0] * (len(hypothesis) + 2) + [1] * (len(premise_1) + 1)

                _truncate_seq_pair(premise_2, hypothesis, max_seq_len - 3)
                input_ids_2 = bert_tokenizer.convert_tokens_to_ids(['[CLS]'] + hypothesis + ['[SEP]'] + premise_2 + ['[SEP]'])
                type_ids_2 = [0] * (len(hypothesis) + 2) + [1] * (len(premise_2) + 1)
...
def main(args):
    ...
    # Winograd/COPA tasks
    ######################################
    winograd_train_path = '/mnt/research/CSE842/FinalProject/Benchmarks/Winograd/train/train.c.txt' # Rahman and Ng training set- will be split into training and dev
    winograd_test1_path = '/mnt/research/CSE842/FinalProject/Benchmarks/Winograd/test1/test1.c.txt' # Rahman and Ng test set
    winograd_test2_path = '/mnt/research/CSE842/FinalProject/Benchmarks/Winograd/test2/WSCollection.xml' # Actual challenge data

    copa_train_path = '/mnt/research/CSE842/FinalProject/Benchmarks/COPA/train/copa-train.xml' # Will be split into training and dev
    copa_test_path = '/mnt/research/CSE842/FinalProject/Benchmarks/COPA/test/copa-test.xml'
    ...
    # NEW DATASETS - Winograd and COPA
    winograd_train_data, winograd_dev_data = load_winograd(winograd_train_path, split=0.90)
    winograd_test1_data = load_winograd(winograd_test1_path)
    winograd_test2_data = load_winograd_xml(winograd_test2_path, 10000)
    winograd_test_data = winograd_test1_data + winograd_test2_data
    logger.info('Loaded {} Winograd train samples'.format(len(winograd_train_data)))
    logger.info('Loaded {} Winograd dev samples'.format(len(winograd_dev_data)))
    logger.info('Loaded {} Winograd test1 samples'.format(len(winograd_test1_data)))
    logger.info('Loaded {} Winograd test2 samples'.format(len(winograd_test2_data)))

    copa_train_data, copa_dev_data = load_copa(copa_train_path, split=0.90)
    copa_test_data = load_copa(copa_test_path)
    ...
    # BUILD Winograd
    winograd_train_fout = os.path.join(mt_dnn_root, 'winograd_train.json')
    winograd_dev_fout = os.path.join(mt_dnn_root, 'winograd_dev.json')
    winograd_test_fout = os.path.join(mt_dnn_root, 'winograd_test.json')
    build_winograd(winograd_train_data, winograd_train_fout)
    build_winograd(winograd_train_data, winograd_dev_fout)
    build_winograd(winograd_test_data, winograd_test_fout)
    logger.info('done with winograd')

    # BUILD COPA
    copa_train_fout = os.path.join(mt_dnn_root, 'copa_train.json')
    copa_dev_fout = os.path.join(mt_dnn_root, 'copa_dev.json')
    copa_test_fout = os.path.join(mt_dnn_root, 'copa_test.json')
    build_copa(copa_train_data, copa_train_fout)
    build_copa(copa_dev_data, copa_dev_fout)
    build_copa(copa_test_data, copa_test_fout)
    logger.info('done with copa')
